...
import { PublicationPreview, SubscriptionType } from '_models';
import { FundraisingService, PaymentService, StatesService } from '_services';
...
export class SubscribeComponent implements OnInit, OnDestroy {
  ...
  chargeSubscription(token) {
    this.disableSubmit = true;
    this.paymentService
      .addSubscription(this.publication.id, this.getSubscriptionData(token))
      .subscribe(
        () => {
          this.disableSubmit = false;
          this.formStatusClass = '';
  ...
  private getSubscriptionData(token) {
    const data = {
      ...
      mailingAddress: {
        addressedTo:
          this.subscriptionForm.get('firstName').value +
          ' ' +
          this.subscriptionForm.get('lastName').value,
        address1: this.subscriptionForm.get('address1').value,
        address2: this.subscriptionForm.get('address2').value
          ? this.subscriptionForm.get('address2').value
          : '',
        city: this.subscriptionForm.get('city').value,
        state: this.subscriptionForm.get('state').value,
        zip: this.subscriptionForm.get('zip').value,
        phone: this.subscriptionForm.get('phoneNumber').value
          ? this.subscriptionForm.get('phoneNumber').value
          : '',
        shipTo: '',
      },
    };
  ...
  private scrollToError() {
    ...
    if (
      this.subscriptionForm.get('firstName').invalid ||
      this.subscriptionForm.get('lastName').invalid
    ) {
      target = '.group-name';
      ...
      this.subscriptionForm.get('city').invalid ||
      this.subscriptionForm.get('state').invalid
    ) {
      target = '.group-state';
    } else {
      target = '.group-zip';
