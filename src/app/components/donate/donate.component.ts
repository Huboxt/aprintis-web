...
import { DonationLevel, PublicationPreview } from '_models';
...
export class DonateComponent implements OnInit, OnDestroy {
  ...
  chargeDonation(token) {
    ...
    this.paymentService
      .addDonation(this.publication.id, this.getDonationData(token))
      .subscribe(
        () => {
  ...
  private getDonationData(token) {
    const data = {
      ...
      mailingAddress: {
        addressedTo:
          this.donationForm.get('firstName').value +
          ' ' +
          this.donationForm.get('lastName').value,
        address1: this.donationForm.get('address1').value,
        address2: this.donationForm.get('address2').value
          ? this.donationForm.get('address2').value
          : '',
        city: this.donationForm.get('city').value,
        state: this.donationForm.get('state').value,
        zip: this.donationForm.get('zip').value,
        phone: this.donationForm.get('phoneNumber').value
          ? this.donationForm.get('phoneNumber').value
          : '',
        shipTo: '',
      },
    };
  ...
  private scrollToError() {
    ...
    if (
      ...
      this.donationForm.get('city').invalid ||
      this.donationForm.get('state').invalid
    ) {
      target = '.group-state';
    } else {
      target = '.group-zip';
