...
import { /*DonationLevel,*/ PageAdPricing, PublicationPreview } from '_models';
import {
  /*FundraisingService,*/ PaymentService,
  StatesService,
  UserService,
} from '_services';
...
export class AdvertiseComponent implements OnInit, OnDestroy {
      ...
      this.authService.isAuthenticated().subscribe(async (auth) => {
        if (!auth) {
          this.router.navigateByUrl(
            `publication/${this.publication.id}/advertise/${this.adPricing.id}/register`
          );
        }
      });
      this.createForm();
  ...
  chargeAd(token) {
    this.disableSubmit = true;
    this.paymentService
      .addPaymentForAd(this.publication.id, this.getAdPaymentData(token))
      .subscribe(
        () => {
          this.disableSubmit = false;
          this.formStatusClass = '';
          ...
          this.card.clear();
          this.userService
            .updateUserRoles(this.authService.getToken())
            .subscribe(() => {
              if (this.statusData) {
                this.router.navigateByUrl(
  ...
  private getAdPaymentData(token) {
    const data = {
      ...
      mailingAddress: {
        addressedTo:
          this.payAdForm.get('firstName').value +
          ' ' +
          this.payAdForm.get('lastName').value,
        address1: this.payAdForm.get('address1').value,
        address2: this.payAdForm.get('address2').value
          ? this.payAdForm.get('address2').value
          : '',
        city: this.payAdForm.get('city').value,
        state: this.payAdForm.get('state').value,
        zip: this.payAdForm.get('zip').value,
        phone: this.payAdForm.get('phoneNumber').value
          ? this.payAdForm.get('phoneNumber').value
          : '',
        shipTo: '',
      },
    };
  ...
  private scrollToError() {
    ...
    if (
      this.payAdForm.get('firstName').invalid ||
      this.payAdForm.get('lastName').invalid
    ) {
      target = '.group-name';
      ...
      this.payAdForm.get('city').invalid ||
      this.payAdForm.get('state').invalid
    ) {
      target = '.group-state';
    } else {
      target = '.group-zip';
