...
import { AdvertisingService, DraftService } from '_services';
...
export class PublicationAdsComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('dialogConfirmAdResource', { static: false })
  dialogConfirmAdResource;
  isBrowser = false;
  ...
  ngOnInit() {
    this.route.data.pipe(untilDestroyed(this)).subscribe((data) => {
      ...
      this.adResources = data.adResources.filter(
        (e) => e.issueId === data.issue.id
      );
      this.loading = false;
    });
  }
  ...
  onSaveAdResource() {
    ...
    this.advertisingService
      .getMyOwnAdResources(this.issue.id)
  ...
  deleteAdResource(adResourceId, $event) {
    ...
    this.dialog
      .open(this.dialogConfirmAdResource, { panelClass: 'confirm-dialog' })
      .afterClosed()
      .pipe(untilDestroyed(this))
      .subscribe((result) => {
        if (result) {
          this.advertisingService
            .deleteAdResource(adResourceId)
            .subscribe(() => {
              this.advertisingService
                .getMyOwnAdResources(this.issue.id)
                .subscribe((response: any) => {
  ...
  openPdf() {
    ...
    this.http.head(this.currentDraft.filePublicUrl).subscribe(
      (data) => {
