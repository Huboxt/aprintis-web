...
import { PaymentMetadata, PaymentMethod } from '_models';
import { Issue } from '_models/issue.model';
import { IssueService, PaymentService, StatesService } from '_services';
...
export class PaymentInfoComponent implements OnInit {
  ...
  initForm() {
    this.issueService.getIssue(this._issueId).subscribe((issue: Issue) => {
      this.issue = issue;
      const defaults = this.issue.paymentMethod
        ? this.issue.paymentMethod
        : new PaymentMethod();
      const paymentDetails = defaults.metadata
        ? defaults.metadata
        : new PaymentMetadata();
      this.paymentGroup.controls['accountHolderType'].setValue(
        paymentDetails.accountHolderType
      );
  ...
  private setForACH(toSet = true) {
    ...
    if (toSet) {
      ...
      this.paymentGroup
        .get('routingNumber')
        .setValidators([Validators.required, this.routingNumberValidator]);
  ...
  createBankAccountChargeData() {
    return this.paymentService.createBankAccountToken({
      // Creates card token
      accountHolderType: this.paymentGroup.get('accountHolderType').value,
      routingNumber: this.paymentGroup.get('routingNumber').value,
      accountNumber: this.paymentGroup.get('accountNumber').value,
